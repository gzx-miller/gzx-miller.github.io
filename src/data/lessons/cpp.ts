  ...
  },
{
    ...
    principle: 'C++ 运算符分为算术运算符、关系运算符、逻辑运算符、位运算符、赋值运算符、条件运算符、逗号运算符、成员访问运算符等。运算符有优先级和结合律，复杂表达式应使用括号明确意图。关于求值顺序：C++17 仅对赋值、复合赋值、&&、||、?:、逗号运算符明确规定了左操作数先于右操作数求值；其余多数运算符的子表达式求值顺序仍未指定，因此在同一表达式中若某个子表达式修改了另一个子表达式将要读取的对象（如 i = i++），其行为是未定义的。整数提升规则：小整数类型（bool、char、short）在表达式中被提升为 int（如果 int 能容纳原类型的所有值）或 unsigned int。算术转换：二元运算符的两个操作数被转换为共同类型，通常涉及整型提升后按类型等级转换。',
    flow: [
  ...
  },
{
    ...
    notes: [
      ...
      'C++11 起，std::string 的 data() 与 c_str() 都保证返回以 \\0 结尾的连续缓冲区（在 s[s.size()] 处写入 \\0 是标准允许的），但不应通过 data()/c_str() 返回的指针改写既有字符，越界读写需用 at() 或范围 for 规避。',
  ...
  },
{
    ...
    notes: [
      ...
      '指针相等比较（==/!=）对指向不同对象的指针是良定义的（结果为 false）；而关系比较（<、<=、>、>=）只有在两个指针指向同一数组的元素时才有意义，否则结果是未指定或实现定义的。',
      '引用不是独立对象，不重新占用存储，因此既不存在“引用的引用”，也不存在“指向引用的指针”：&r 得到的正是 r 所绑定对象的地址，int& *p 是非法语法，正确写法是 int* p = &r。',
  ...
  },
{
    ...
    notes: [
      ...
      'this 的类型是 类名* const，即指针本身是 const（不能改为指向其他对象）、所指向的是非 const 类对象；在 const 成员函数中类型为 const 类名* const（指向 const 对象，故其中只能调用其他 const 成员函数）。',
  ...
  },
{
    ...
    flow: [
      ...
      '理解特殊成员函数：默认构造、析构、拷贝构造、拷贝赋值，以及 C++11 起新增的移动构造、移动赋值。',
  ...
  },
{
    ...
    principle: 'Lambda 表达式（匿名函数）可以在需要函数对象的地方定义内联函数，避免定义命名函数或函数对象的样板代码。Lambda 语法：[capture](params) -> ret { body }。捕获列表指定 lambda 可以使用哪些外部变量以及如何捕获（值捕获 [x] 或引用捕获 [&x]；[=] 值捕获所有自动存储期变量；[&] 引用捕获所有自动存储期变量）。默认情况下，值捕获的变量在 lambda 内是 const 的（不能修改），除非 lambda 标记为 mutable。C++14 允许泛型 lambda（参数用 auto）；C++14 允许初始化捕获（[x = std::move(src)] 可以把移动引入捕获）；模板 lambda（携带显式模板参数列表，推演与约束更精确）自 C++20 起可用。',
    flow: [
  ...
  },
{
    ...
    flow: [
      ...
      'C++17 起 noexcept 属于函数的类型，会影响函数指针类型与重载决议；对不抛异常的函数声明 noexcept，可让标准库在移动等操作中采用更高效的实现路径。',
    ],
    notes: [
  ...
  },
{
    ...
    notes: [
      ...
      'C++14 起 constexpr 函数体可以包含局部变量声明、循环、条件分支与 switch 等语句，但仍必须是纯表达式计算，不允许包含 new/delete、throw、goto、static 或局部静态变量等会引入副作用或非 constexpr 的行。',
  ...
  },
{
    ...
    principle: '良好的编码规范和工程实践可以提高代码质量、可读性和可维护性。C++ 有一些著名的编码规范：C++ Core Guidelines（C++ 之父 Bjarne Stroustrup 和 Herb Sutter 主导，涵盖类型安全、接口设计、资源管理、并发等）、Google C++ Style Guide、MISRA C++（汽车工业）。核心原则：使用 RAII 和智能指针管理资源（避免裸 new/delete）；优先使用标准库（不要重复造轮子）；使用 const 正确（帮助编译器优化，自文档化）；避免使用宏（用 constexpr、enum、template 替代）；理解对象生命周期和所有权；使用工具（静态分析、sanitizers、clang-tidy）捕获错误。构建系统：CMake 是现代 C++ 事实标准；包管理：vcpkg、Conan；测试：Google Test、Catch2。',
    flow: [
