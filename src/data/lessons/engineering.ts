...
export const lessons: Lesson[] = [
  {
    ...
    demo: G01EnvironmentConfig,
    code: () => Promise.resolve(`# .env.development - 开发环境配置
# VITE_ 前缀的变量会暴露到客户端
VITE_API_BASE_URL=http://localhost:3000/api
VITE_APP_TITLE=开发环境 - 应用
...
`),
  ...
  },
  {
    ...
    demo: G02CodeQuality,
    code: () => Promise.resolve(`// eslint.config.js - ESLint 配置
...
`),
  ...
  },
  {
    ...
    demo: G03UnitTesting,
    code: () => Promise.resolve(`// discount.test.ts - 折扣计算单元测试
import { describe, it, expect } from 'vitest'
import { calculateDiscount } from './discount'

describe('calculateDiscount', () => {
  it('满 200 减 50，金额刚好 200 时生效', () => {
    const result = calculateDiscount(200, { type: 'full', threshold: 200, amount: 50 })
    expect(result).toBe(150)
  })

  it('未满阈值时不打折', () => {
    const result = calculateDiscount(199, { type: 'full', threshold: 200, amount: 50 })
    expect(result).toBe(199)
  })

  it('折扣后金额不能为负数', () => {
    const result = calculateDiscount(30, { type: 'full', threshold: 20, amount: 50 })
    expect(result).toBe(0)
  })

  it('百分比折扣按比例计算', () => {
    const result = calculateDiscount(100, { type: 'percent', rate: 0.8 })
...
`),
  ...
  },
  {
    ...
    demo: G04ComponentTesting,
    code: () => Promise.resolve(`// LoginForm.test.ts - 组件交互测试
import { describe, it, expect } from 'vitest'
...
describe('LoginForm', () => {
  ...
  it('输入用户名和密码后按钮可点击', async () => {
    const wrapper = mount(LoginForm)
    await wrapper.find('input[aria-label="用户名"]').setValue('testuser')
    await wrapper.find('input[aria-label="密码"]').setValue('pass123')
    const submitBtn = wrapper.find('button[type="submit"]')
    expect(submitBtn.attributes('disabled')).toBeUndefined()
  })

  it('提交时触发 submit 事件并传递表单数据', async () => {
    const wrapper = mount(LoginForm)
    await wrapper.find('input[aria-label="用户名"]').setValue('testuser')
    await wrapper.find('input[aria-label="密码"]').setValue('pass123')
...
`),
  ...
  },
  {
    ...
    demo: G05CIPipeline,
    code: () => Promise.resolve(`# .github/workflows/ci.yml - CI 流水线配置
name: CI
...
`),
  ...
  },
  {
    ...
    demo: G06PerformanceBudget,
    code: () => Promise.resolve(`// size-limit.config.js - 性能预算配置
export default [
  {
    name: '主应用入口',
...
`),
  ...
  },
  {
    ...
    demo: G07Accessibility,
    code: () => Promise.resolve(`// a11y.test.ts - 无障碍自动化测试
import { describe, it, expect } from 'vitest'
import { mount } from '@vue/test-utils'
...
describe('无障碍测试', () => {
  ...
  it('按钮使用原生 button 元素而非 div', () => {
    const wrapper = mount(LoginForm)
    const submitBtn = wrapper.find('[role="button"]')
...
`),
  ...
  },
  {
    ...
    demo: G08SecurityDelivery,
    code: () => Promise.resolve(`# _headers - Netlify 安全响应头配置
/*
...
# 内容安全策略 - 先报告模式观察
/*.html
  Content-Security-Policy-Report-Only: default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://api.example.com; report-uri /csp-report
...
`),
  ...
  },
  {
    ...
    demo: G09BuildPlugin,
    code: () => Promise.resolve(`// vite-plugin-example.ts - 自定义 Vite 插件
...
export function myPlugin(): Plugin {
  return {
    ...
    // 配置阶段：修改 Vite 配置
    config(config, env) {
      console.log('config hook:', env.mode)
      return {
        resolve: {
          alias: { '@': '/src' },
        },
...
`),
  ...
  },
  {
    ...
    demo: G10E2eTesting,
    code: () => Promise.resolve(`// signup.spec.ts - 端到端测试
...
test.describe('报名流程', () => {
  ...
  test('完整的报名流程', async ({ page }) => {
    ...
    // 选择套餐
    await page.getByRole('radio', { name: '高级版' }).check()
  ...
  test('表单验证：必填项为空时显示错误', async ({ page }) => {
    await page.getByRole('button', { name: '提交报名' }).click()
...
`),
  ...
  },
  {
    ...
    demo: G11BundleAnalysis,
    code: () => Promise.resolve(`// vite.config.ts - 产物分析配置
...
`),
  ...
  },
  {
    ...
    demo: G12Monorepo,
    code: () => Promise.resolve(`# pnpm-workspace.yaml - Monorepo 工作区配置
packages:
  - 'apps/*'
  - 'packages/*'
  - 'tools/*'
...
`),
  ...
  },
  {
    ...
    demo: G13Docker,
    code: () => Promise.resolve(`# Dockerfile - 多阶段构建
# 第一阶段：构建
...
`),
  ...
  },
  {
    ...
    demo: G14GitWorkflow,
    code: () => Promise.resolve(`// commitlint.config.js - 提交信息规范配置
export default {
...
`),
  ...
  },
  {
    ...
    demo: G15I18n,
    code: () => Promise.resolve(`// src/i18n/index.ts - 国际化配置
import { createI18n } from 'vue-i18n'

const messages = {
  zh: {
    welcome: '欢迎，{name}',
    items: '{count, plural, =0 {没有项目} one {1 个项目} other {# 个项目}}',
    nav: {
      home: '首页',
      about: '关于',
    },
...
`),
  ...
  },
  {
    ...
    demo: G16MicroFrontend,
    code: () => Promise.resolve(`// vite.config.ts - Module Federation 配置（宿主应用）
import { defineConfig } from 'vite'
import vue from '@vitejs/plugin-vue'
...
export default defineConfig({
  plugins: [
    ...
    federation({
      name: 'host-app',
      remotes: {
        cartApp: 'http://localhost:5001/assets/remoteEntry.js',
...
`),
  ...
  },
  {
    ...
    demo: G17PnpmWorkspaces,
    code: () => Promise.resolve(`# pnpm-workspace.yaml - pnpm 工作区配置
packages:
  - 'apps/*'
  - 'packages/*'
...
# 包引用示例 - packages/button/package.json
# {
#   "name": "@acme/button",
#   "version": "1.0.0",
#   "dependencies": {
#     "@acme/utils": "workspace:*",
#     "@acme/icons": "workspace:^1.0.0"
#   }
# }
...
# pnpm --filter @acme/button test  # 只运行指定包的测试
# pnpm add lodash -D --filter @acme/utils  # 给指定包加依赖
`),
    language: 'yaml',
  ...
  },
  {
    ...
    demo: G18Turborepo,
    code: () => Promise.resolve(`// turbo.json - Turborepo 配置
{
...
`),
  ...
  },
  {
    ...
    demo: G19Changesets,
    code: () => Promise.resolve(`// .changeset/config.json - Changesets 配置
{
...
# 示例 changeset 文件 - .changeset/blue-elephants-dance.md
# ---
# "@acme/ui": minor
# "@acme/utils": patch
# ---
#
# 新增 Button 组件 variant 属性
#
# - 支持 primary / secondary / outline 三种样式
# - 修复 Utils 工具函数的类型错误

# 常用命令：
# pnpm changeset add      # 创建新的 changeset
# pnpm changeset version  # 汇总变更并更新版本号
# pnpm changeset publish  # 发布更新的包
`),
    language: 'javascript',
  ...
  },
  {
    ...
    demo: G20Storybook,
    code: () => Promise.resolve(`// src/components/Button.stories.ts - Button 组件 Stories
...
`),
  ...
  },
  {
    ...
    demo: G21Chromatic,
    code: () => Promise.resolve(`# .github/workflows/chromatic.yml - Chromatic CI 配置
name: Chromatic
...
`),
  ...
  },
  {
    ...
    demo: G22Playwright,
    code: () => Promise.resolve(`// playwright.config.ts - Playwright 配置
...
`),
  ...
  },
  {
    ...
    demo: G23VitestConfig,
    code: () => Promise.resolve(`// vitest.config.ts - Vitest 高级配置
...
`),
  ...
  },
  {
    ...
    demo: G24NxWorkspace,
    code: () => Promise.resolve(`// nx.json - Nx 工作区配置
{
...
`),
  ...
  },
  {
    ...
    demo: G25BundleAnalyzer,
    code: () => Promise.resolve(`// vite.config.ts - 体积分析与代码分割配置
import { defineConfig } from 'vite'
import vue from '@vitejs/plugin-vue'
import { visualizer } from 'rollup-plugin-visualizer'
...
`),
  ...
  },
  {
    ...
    demo: G26Pwa,
    code: () => Promise.resolve(`// vite.config.ts - PWA 配置
import { defineConfig } from 'vite'
import vue from '@vitejs/plugin-vue'
