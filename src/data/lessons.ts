...
export const knowledgeCategories: KnowledgeCategory[] = [
  ...
  { id: 'llm-principles', name: 'LLM 原理', path: '/llm-principles', status: 'ready', intro: '从向量到 Transformer 的大模型原理体系。本分类循「先给朴素方案、再发现不足、迭代改好」的路径，把向量、神经网络、注意力机制与预训练范式逐一手把手逼出来，最终看懂 ChatGPT 是怎么长成的。' },
  ...
  { id: 'uni-app', name: 'uni-app', path: '/uni-app', status: 'ready', intro: 'uni-app 是使用 Vue.js 语法开发多端应用的框架，一套代码可发布到 H5、微信/支付宝等小程序与 App。本分类从项目结构与分包、条件编译、页面与应用生命周期、内置组件、路由跳转、rpx 适配、easycom、交互反馈、组合式 API 与数据请求出发，覆盖跨端开发的核心能力。', officialUrl: 'https://uniapp.dcloud.net.cn/' },
]
